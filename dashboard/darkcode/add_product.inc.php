	  ...
	  if (empty($category)) {
	   ...
	   elseif( empty($productname)){
		...
		//check if phone is valid			
		       ...
		       header("Location: ../add_product.php? please upload a picture of the product");
               exit();
		   	   }
			   else{ 
        			   if (in_array($required_img_Actual_Ext, $allowed_Ext)) {
        					...
        					$sql ="INSERT INTO product (seller_id, product_name, product_category,  product_img_req, product_img_1, product_img_2, product_img_3, remark, product_desc, price, size, tag, time) VALUES
							                                                     ('$seller_id', '$productname',  '$category', '$required_img_NameNew', '', '', '', '$remark', '$description', '$price', '$size', '$tag', '$time')";
